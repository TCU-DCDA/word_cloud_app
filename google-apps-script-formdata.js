/**
 * Google Apps Script for Word Cloud App - FORM DATA VERSION
...
function handleRequest(e, method) {
  try {
    ...
    // Handle POST request (submit data) - check both postData and parameters
    if (method === 'POST') {
      ...
      // Fallback to JSON parsing if available
        ...
        console.log('Trying to parse JSON from postData');
        try {
...
function submitResponse(feelings, timestamp) {
  try {
    ...
    if (!feelings || feelings.toString().trim() === '') {
      return {
        success: false,
    ...
    const feelingsText = feelings.toString().trim();
    const wordCount = feelingsText.split(/\s+/).length;
    
    // Add row to sheet
    const newRow = [submissionTime, feelingsText, wordCount];
